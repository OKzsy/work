    ...
    for iyoffset in range(ysize):
        for ixoffset in range(xsize):
            ...
            for iflag in range(num_flag):
                flag_p[iyoffset, ixoffset, iflag] = calc_prob(isample_data / 10000, standard_array[iflag, :] / 10000)
...
def run_get_prob(in_list):
    ...
    for in_file in in_files:
        ...
        in_data = sds.ReadAsArray(0, 0, xsize, ysize)

        flag_p = get_prob(in_data, standard_spec_array)
...
def search_flag(folder_path, file_extension):
    ...
    for dir_path, dir_names, files in os.walk(folder_path):
        for file in files:
            if file.lower().endswith(file_extension):
                flag.append(str(os.path.splitext(file)[0].split('-')[-1]).split('_')[-1])
                search_files.append(os.path.normpath(os.path.join(dir_path, file)))
    ...
    for iflag in flag_set:
        ...
        for ifile in search_files:
            file_flag = str(os.path.splitext(ifile)[0].split('-')[-1]).split('_')[-1]
            if file_flag == iflag:
...
def main():
    ...
    for iflag in flag_list:
        ...
        for iflag_file in flag_files:
            ...
            get_spec(source_dataset.ReadAsArray(0, 0, xsize, ysize), iout_file)
    ...
    # in_list = tif_files + [standard_spec_array, out_prob_dir]

    # run_get_prob(in_list)

    prob_files = search_file(out_prob_dir, '.tif')
    file_info_list = read_raster(in_file)
    # flag_p = get_prob(in_data, np.array(standard_spec).reshape(1, len(standard_spec)))
    # #
    out_driver = gdal.GetDriverByName('GTiff')
    # # #
    if os.path.exists(out_p_file):
        out_driver.Delete(out_p_file)
    out_dataset = out_driver.Create(out_p_file, file_info_list[0], file_info_list[1], standard_spec_array.shape[0], gdal.GDT_Float32)
...
if __name__ == '__main__':
    ...
    in_file = r'D:\Data\Test_data\classification\20180815\GF2_20180327_S2_0309_0324_0408_0418_sub.tif'
    in_dir = r'D:\Data\Test_data\classification\20180815\sample_tif'
    out_csv_dir = r'D:\Data\Test_data\classification\20180815\GF2_csv'
    block_dir = r'D:\Data\Test_data\classification\20180815\block_tif_sub'
    out_csv_file = r'D:\Data\Test_data\classification\20180815\yingqiao_mean.csv'
    out_p_file = r'D:\Data\Test_data\classification\20180815\GF2_20180327_S2_0309_0324_0408_0418_p2.tif'
    out_prob_dir = r'D:\Data\Test_data\classification\20180815\block_tif_sub_prob'

    #
    # in_file = r"E:\大蒜样本\GF2_20180327_sha1.tif"
    # in_dir = r'E:\大蒜样本\sample_tif'
    # out_csv_dir = r'E:\大蒜样本\GF2_csv'
    # block_dir = r'E:\大蒜样本\block_tif'
    # out_prob_dir = r'E:\大蒜样本\block_tif_prob'
    # out_csv_file = r'E:\大蒜样本\standard_mean.csv'
    # out_p_file = r"E:\大蒜样本\GF2_20180327_sha1_prob.tif"
