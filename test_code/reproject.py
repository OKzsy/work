...
def def_prj(src_dst, prj, seq):
    # 根据目标大小，在内存创建结果影像
    tmp_dst_path = r'/vsimem/tmp_dst_{}.tiff'.format(str(seq))
    print('Begin to define the projection!')
    gdal.Translate(tmp_dst_path, src_dst, format='GTiff', outputSRS=prj, callback=progress)
...
def reproject_dataset(src_ds, count):
    """
    ...
    src_dst_prj = def_prj(src_ds, odefprj, count)
    src_ds = None
    # 获取原始投影
...
def main(in_dir, out_dir, partfileinfo):
    ...
    for ifile in Pending_images:
        ...
        # 重投影
        dest = reproject_dataset(dataset, count)
        # 拼接输出影像绝对路径
