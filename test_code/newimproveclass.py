...
def main(file, shp, out):
    ...
    for feat in re_shp_l:
        ...
        real_overlap = np.choose(mask_array, (overlap, 0))
        mem_ds.GetRasterBand(1).WriteArray(real_overlap, int(offset[0]), int(offset[1]))
        mask_ds = None
        feat_ds = None
...
if __name__ == '__main__':
    ...
    in_file = r"\\192.168.0.234\nydsj\user\花椒_监督分类\石蟆镇_huajiao1.tif"
    in_shp = r"\\192.168.0.234\nydsj\user\LXX\重庆江津\chongqing\huajiao_google_20180720_2_江津区_石蟆镇.shp"
    outpath = r"\\192.168.0.234\nydsj\user\花椒_监督分类\石蟆镇_huajiao.tif"
    # in_file = sys.argv[1]
    # in_shp = sys.argv[2]
    # outpath = sys.argv[3]
