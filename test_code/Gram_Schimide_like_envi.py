...
def phi(mss_band, gst_band, mean):
    ...
    cov = np.nansum(mss_band) / (mss_band.shape[0] * mss_band.shape[1] - np.isnan(mss_band).sum())
    # 计算gst_band的方差
    variance1 = np.nansum((gst_band - gst_band_mean) ** 2) /(gst_band.shape[0] * gst_band.shape[1] - np.isnan(gst_band).sum())
...
def Gram_Schmidt_Transform(simulated_pan_dataset, mss_dataset, GST_img_path):
    ...
    zero_index = np.where(simulated_pan_array == 0)
    zero_number = zero_index[0].shape[0]
    ...
    for iband in range(1, bandcount):
        ...
        for iGSband in range(iband):
            if iGSband == 0:
                phi_gs = phi(mss_array, simulated_pan_array, mean=mss_mean) * simulated_pan_array
            else:
...
def modify_pan_stat(pan_dataset, simulated_pan_dataset, coordinate):
    ...
    simulated_array = simulated_pan_dataset.ReadAsArray().astype(np.float64)
    simulated_array = np.where(simulated_array == 0, np.nan, simulated_array)

    pan_array = pan_dataset.ReadAsArray(offset[0], offset[1], coordinate[2], coordinate[3],).astype(np.float64)
...
def Inv_Gram_Schmidt_Transform(modified_pan, GST_file, mss_resize_file, coordinate, fusion):
    # 获取数据的有效区域
    ...
    for iband in range(1, bandcount):
        ...
        for iGSband in range(iband):
            if iGSband == 0:
                phi_gs = phi(mss_resize_array, modified_pan, mean=mss_resize_mean) * modified_pan
            else:
                temp_GST_arr = np.where(GST_array[iGSband - 1, :, :] == 0, np.nan, GST_array[iGSband - 1, :, :])
                # GST_array[iGSband - 1, :, :][index] = np.nan
                # temp_GST_arr = GST_array[iGSband - 1, :, :]
                phi_gs += phi(mss_resize_array, temp_GST_arr, mean=mss_resize_mean) * temp_GST_arr
...
def main(pan, mss, fusion):
    ...
    print(time.clock())
    gc.collect()
    Inv_Gram_Schmidt_Transform(modified_pan_arr, GST_resize_file, mss_resize_file, coordinate, fusion)
    # 删除临时文件
...
if __name__ == '__main__':
    ...
    fusion_path = r"F:\test_data\GS_test\GF2_PMS2_E114.0_N32.2_20180718_L1A0003330812-py_sha1.tif"
