...
# -*- coding:utf-8 -*-
"""
# @Time    : 2019/5/13 13:53
# @Author  : zhaoss
# @FileName: label_location.py
...
def add_sample(dom, keywords):
    ...
    dom = add_child(dom, "annotation", "Transform")
    dom = add_child(dom, "Transform", "longitude", keywords["longitude"])
    dom = add_child(dom, "Transform", "XResolution", keywords["XResolution"])
    dom = add_child(dom, "Transform", "XRotate", keywords["XRotate"])
    dom = add_child(dom, "Transform", "latitude", keywords["latitude"])
    dom = add_child(dom, "Transform", "YRotate", keywords["YRotate"])
    dom = add_child(dom, "Transform", "YResolution", keywords["YResolution"])
    return dom
...
def calculate_offset(sample_up_left_corner, corner, resolution):
    offset_x = int((corner[0] - sample_up_left_corner[0]) / resolution[0])
    offset_y = int((corner[1] - sample_up_left_corner[1]) / resolution[1])
    return offset_x, offset_y


def main(sample_shp, lable_shp, xml, img, out):
    """
    ...
    for feat in sample_lyr:
        ...
        # xml路径
        xml_name = os.path.join(xml, feat.GetField(fieldName) + '.xml')
        # 要素提取为图层
        ...
        # 增加sample
        sample_dic = {}
        sample_dic["folder"] = out
        sample_dic["filename"] = os.path.basename(out_img_name)
        sample_dic["path"] = out_img_name
        sample_dic["database"] = "Unknown"
        sample_dic["width"] = str(offset[2] - offset[0])
        sample_dic["height"] = str(offset[3] - offset[1])
        sample_dic["depth"] = "3"
        sample_dic["segmented"] = "0"
        sample_dic["longitude"] = str(geo[0])
        sample_dic["XResolution"] = str(geo[1])
        sample_dic["XRotate"] = str(geo[2])
        sample_dic["latitude"] = str(geo[3])
        sample_dic["YRotate"] = str(geo[4])
        sample_dic["YResolution"] = str(geo[5])
        # 写入xml
        ...
        for lable_feat in lable_lyr:
            # 获取几何要素
            lable_geom_ext = lable_feat.geometry().GetEnvelope()
            up_left_corner = [lable_geom_ext[0], lable_geom_ext[3]]
            down_right_corner = [lable_geom_ext[1], lable_geom_ext[2]]
            corner_min = calculate_offset(sample_ul, up_left_corner, res)
            cornet_max = calculate_offset(sample_ul, down_right_corner, res)
            # 写入xml
            # 增加label标签
            label_dic = {}
            label_dic["name"] = label_name[lable_feat.GetField("obj_lab_1")][lable_feat.GetField("obj_lab_2")]
            label_dic["pose"] = "Unspecified"
            label_dic["truncated"] = "0"
            label_dic["difficult"] = "0"
            label_dic["xmin"] = str(corner_min[0])
            label_dic["ymin"] = str(corner_min[1])
            label_dic["xmax"] = str(cornet_max[0])
            label_dic["ymax"] = str(cornet_max[1])
...
if __name__ == '__main__':
    ...
    main(sample_shp=in_sample_shp, lable_shp=in_lable_shp, xml=xml_path, img=img_path, out=img_out_dir)
    end_time = time.clock()
