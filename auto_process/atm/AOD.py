...
def hdf2tiff(date, latlng, save_path, stitch_path, aod_dir):
    ...
    # os.system("stitch.bat")
    subprocess.call("stitch.bat", shell=True)
    return None


def main(date, latlng, save_path, aod_dir):
    instance = Nasa(date=date, latlng=latlng, save_path=save_path)
    # 爬取符合要求的modis产品名称和数据路径
    modis_product = instance.spider()
    # 下载数据
    status = sync(source=modis_product, dest=save_path, m=instance.mm)
    if status != 1:
        sys.exit("文件没有下载完全，请重新下载")
    # 获取当前工作路径
    function_position = os.path.dirname(os.path.abspath(sys.argv[0]))
    hdf2tiff(date, latlng, save_path, function_position, aod_dir)
    return None
...
if __name__ == '__main__':
    ...
    save_path = os.path.join(r"F:\test", day.replace("-", ""))
    aod_dir = r"F:\test\aod"
    main(date=day, latlng=latlng, save_path=save_path, aod_dir=aod_dir)
    end_time = time.clock()
