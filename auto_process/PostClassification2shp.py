...
# -*- coding:utf-8 -*-
"""
# @Time    : 2019/9/11 12:32
# @Author  : zhaoss
# @FileName: PostClassification2shp.py
...
def main(file, shp, flags, threshold_value, nodata, out):
    ...
    for feat in re_shp_l:
        ...
        if max_pre >= threshold_value:
            ...
            temp_geom = feat.geometry().Clone()
            temp_geom.TransformTo(area_srs)
            out_feat.SetField('classify', max_flag)
            land_area = str(round(temp_geom.GetArea(), 3))
            out_feat.SetField('area', land_area)
            ...
            temp_geom = feat.geometry().Clone()
            temp_geom.TransformTo(area_srs)
            out_feat.SetField('classify', nodata)
            land_area = str(round(temp_geom.GetArea(), 3))
            out_feat.SetField('area', land_area)
            out_lyr.CreateFeature(out_feat)
            temp_geom.Destroy()
...
if __name__ == '__main__':
    ...
    outpath = r"\\192.168.0.234\nydsj\user\ZSS\20190830\result.shp"
    category_flags = [30]
    threshold = 0.5
    nodata = 0
