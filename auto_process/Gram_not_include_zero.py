...
# -*- coding:utf-8 -*-
"""
# @Time    : 2019/9/24 12:36
# @Author  : zhaoss
# @FileName: Gram_not_include_zero.py
...
def Gram_Schmidt_Transform(simulated_pan_dataset, mss_dataset, GST_img_path):
    ...
    GST = np.zeros((bandcount - 1, ysize, xsize), dtype=np.float32, order="C")
    # 第一分量保持不变
    ...
    for iGSTband in range(bandcount - 1):
        GST_ds.GetRasterBand(iGSTband + 1).WriteArray(GST[iGSTband, :, :].astype(np.float32))
    GST_ds.FlushCache()
    GST_ds = None
    GST = None
...
def Inv_Gram_Schmidt_Transform(modified_pan, GST_resize_file, trans_coffic, coordinate, zero_index, fusion):
    ...
    for iband in range(1, bandcount):
        ...
        fusion_ds.GetRasterBand(iband).FlushCache()
        temp_arr = None
        gc.collect()
