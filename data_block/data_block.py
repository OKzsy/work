...
def main(in_file):
    ...
    # 创建输出影像
    out_img = r"F:\test_data\new_test\GF2_20190509_L1A0003988007P_out.tif"
    ...
    if residue == 0:
        numsblocks = ysize // linesInblock
    else:
        numsblocks = (ysize // linesInblock) + 1
    print("all block: {}".format(numsblocks))
    for IDblock in range(numsblocks):
        overlap_line = 15
        dims_get, dims_put = block(overlap_line=overlap_line, totalblocknum=numsblocks, IDblock=IDblock,
                                   linesblock=linesInblock,
