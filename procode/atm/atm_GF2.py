...
def ATM_CORRECT(img_in_path, img_out_path, atm_coe, senID, satID, year):
    ...
    rad_coe_dict = {'GF1': \
                        {'2016': \
                         ...
                         'other': \
                             {'PMS1': [0.1228, 0.1424, 0.1177, 0.1194, 0.1135], \
                              'PMS2': [0.1365, 0.1460, 0.1248, 0.1274, 0.1255]}}, \
                    'GF2': {'2016': \
...
def main(py_path, file_path, partfileinfo='*.tif', AOD=0.1696):
    # 注册所有gdal的驱动
    gdal.AllRegister()
    ...
    for num_file in range(len(original_imgs)):
        ...
        # 输出辐射校正系数
        outcoe = function_position + os.sep + '6SV' + os.sep + 'outcoe' + os.sep + SensorID + '-' + ProductID + '.txt'
