...
def mask_raster(raster_ds, mask_ds, outfile, ext):
    ...
    for band in range(bandCount):
        ...
        banddata = np.choose(mask, (banddata, 200))
        result_ds.GetRasterBand(band + 1).WriteArray(banddata)
        # 计算面积
        pixl_num = ne.where(banddata == 0)[0].shape[0]
        aera = pixl_num * raster_geo[1] * 10**5 * (-raster_geo[5]) * 10**5 / 666.67
    return aera


def main(raster, shp, out, field):
    # 打开栅格和矢量影像
    ...
    # 创建面积文件
    txtpath = os.path.join(out, 'aera') + '.txt'
    f = open(txtpath, 'w', newline=None)
    for feat in re_shp_l:
        ...
        #     os.makedirs(outdir)
        # 村名
        country_name = feat.GetField('{}'.format(field))
        outpath = os.path.join(out, country_name + '.tiff')
        # 要素提取为图层
        ...
        # 记录面积
        f.write('{}  {:<.4f}  {}'.format(country_name, res, '\n'))
        f.flush()
...
if __name__ == '__main__':
    ...
    # # field_id = sys.argv[4]
    field_id = 'XZQMC'
    main(in_file, shpfile, outfile, field_id)
    end_time = time.clock()
