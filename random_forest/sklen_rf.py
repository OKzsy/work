...
class decisionnode:
    def __init__(self, d=None, thre=None, results=None, NH=None, lb=None,
                 rb=None, max_label=None):
        self.d = d  # d表示维度
...
def buildtree(X, y, method='Gini'):
    ...
    if y.size > 1:
        ...
        if (Gain_max > 0 and method == 'GainEnt') or (
                Gain_max >= 0 and len(list(set(y))) > 1 and method == 'Gini'):
            ...
            return decisionnode(d=d, thre=thre, NH=nh, lb=left_branch,
                                rb=right_branch, max_label=max_label)
        else:
            nh = NtHt(y)
            max_label = maxlabel(y)
