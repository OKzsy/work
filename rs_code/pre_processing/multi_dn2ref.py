...
def get_20_jp2(in_list):
    # 20m排序
    jp2_20_list = ['B02_20', 'B03_20', 'B04_20',
                   'B05_20', 'B06_20', 'B07_20', 'B8A_20', 'SCL_20m']
    ...
    for dir_path, dir_names, files in os.walk(folder_path):
        for file in files:
            if file.lower().endswith(file_extension):
                search_files.append(os.path.normpath(
...
def reproj_resample(in_file, match_file, out_file):
    ...
    out_dataset = driver.Create(
        out_file, out_xsize, out_ysize, num_band, data_type)
    ...
    gdal.ReprojectImage(source_dataset, out_dataset,
                        in_proj, match_proj, gdal.GRA_Bilinear)
...
def main(in_dir, out_dir):
    ...
    for izip in zip_files:
        information.append(pool.apply_async(func, args=(izip,)))
    pool.close()
    pool.join()
    for res in information:
        print("the information is:{}".format(res.get()))
    ...
    #         # dn2ref(izip, out_dir)
    #         p = mp.Process(target=dn2ref, args=(izip, out_dir,))
    #         p.start()
    #         process_list.append(p)
    #
    #     for it in process_list:
    #         it.join()
